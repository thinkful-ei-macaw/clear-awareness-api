    ...
    trx.raw(
      ...
      .then(() =>
        ...
        //   trx.raw(`ALTER SEQUENCE language_id_seq minvalue 0 START WITH 1`),
          trx.raw(`ALTER SEQUENCE user_table_id_seq minvalue 0 START WITH 1`),
        //   trx.raw(`SELECT setval('word_id_seq', 0)`),
        //   trx.raw(`SELECT setval('language_id_seq', 0)`),
          trx.raw(`SELECT setval('user_table_id_seq', 0)`),
        ])
      )
  )
}
...
function seedUsers(db, users) {
  ...
  return db.transaction(async trx => {
    await trx.into('user_table').insert(preppedUsers)

    await trx.raw(
      `SELECT setval('user_table_id_seq', ?)`,
